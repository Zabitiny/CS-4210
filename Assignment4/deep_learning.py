...
# SPECIFICATION: Classifying clothing articles using neural networks
...
for h in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for n in n_neurons:                     #looking or the best parameters w.r.t the number of neurons
        for l in l_rate:                    #looking or the best parameters w.r.t the learning rate
            ...
            model = build_model(n_hidden=h, n_neurons_hidden=n, n_neurons_output=10, learning_rate=l)
            ...
            #-->add your Pyhton code here
            accuracy = 0
            for prediciton, truth in zip(class_predicted, y_test):
                if prediciton == truth:
                    accuracy += 1
