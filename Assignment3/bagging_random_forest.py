...
# SPECIFICATION: Practice bagging
...
with open('optdigits.tes', 'r') as testingFile:
  ...
  for k in range(20): #we will create 20 bootstrap samples here (k = 20). One classifier will be created for each bootstrap sample
    ...
    for i, testSample in enumerate(dbTest):
        ...
        for j, vote in enumerate(classVotes[i]):
            if int(clf.predict([testSample[:-1]])[0]) == j:
                classVotes[i][j] += 1
        
        if k == 0: #for only the first base classifier, compare the prediction with the true label of the test sample here to start calculating its accuracy
            #--> add your Python code here
            if clf.predict([testSample[:-1]])[0] == testSample[-1]:
                accuracy += 1
  ...
  for i, row in enumerate(dbTest):
      if int(row[-1]) == classVotes[i].index(max(classVotes[i])):
          accuracy += 1
  ...
  accuaracy = 0
  for row in dbTest:
    classPredictedRf = clf.predict([row[:-1]])
    #compare the Random Forest prediction for each test sample with the ground truth label to calculate its accuracy
    #--> add your Python code here
    if classPredictedRf == row[-1]:
        accuracy += 1
